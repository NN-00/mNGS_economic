...
F2_A_plot <- ggplot(F2_A_data, aes(x = Method, y = Count, fill = Method)) +
  ...
  geom_signif(
    ...
    y_position = max(F2_A_data$Count) * 1.1,
    tip_length = 0.01
  ) +
  annotate(
    "text", x = 1.5, y = max(F2_A_data$Count) * 1.15,
...
## Fig.2 B
F2_B_data <- read_excel("00-Final-dataset-mNGS_economic.xlsx", sheet = "Sheet3")
