...
if("antibiotic" %in% colnames(mNGS_time_long)){
  mNGS_time_long <- mNGS_time_long %>% select(-antibiotic)
}

lower_bound_cost2 <- quantile(mNGS_time$cost, 0.05, na.rm=TRUE)
upper_bound_cost2 <- quantile(mNGS_time$cost, 0.95, na.rm=TRUE)
...
max_cost_limit <- max(mNGS_time_cleaned_cost_long$cost, na.rm = TRUE)
...
Fig_5_B <- ggplot(mNGS_time_cleaned_cost_long, aes(x = group, y = cost, fill = group)) +
  ...
  scale_y_continuous(breaks = seq(0, max_cost_limit, by = 10000)) +
  theme_minimal() +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid.major = element_blank(),  
